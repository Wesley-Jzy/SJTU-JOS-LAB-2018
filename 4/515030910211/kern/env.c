...
env_setup_vm(struct Env *e)
{
	...
	/* init env_pgdir, use kern_pgdir as a template */
...
env_alloc(struct Env **newenv_store, envid_t parent_id)
{
	...
	/* Lab4 Challenge */
	e->env_priority = ENV_PRIORITY_LOW;
...
region_alloc(struct Env *e, void *va, size_t len)
{
	...
	for (off = 0; off < size; off+=PGSIZE) {
		...
		if (!pp) {
			panic("region_alloc(): pp = NULL\n");
		}
		pp->pp_ref++;

		if (page_insert(e->env_pgdir, pp, 
			(char *)vaddr + off, PTE_W | PTE_U) < 0) {
			panic("region_alloc(): page table cannot be allocated\n");
		}
	}
}
...
load_icode(struct Env *e, uint8_t *binary, size_t size)
{
	...
	for (; ph < eph; ph++) {
		if (ph->p_type == ELF_PROG_LOAD) {
			/* use empty mem */
			if (ph->p_va + ph->p_memsz > e->env_sbrk) {
				e->env_sbrk = ROUNDUP(ph->p_va + ph->p_memsz, PGSIZE);
			}

			/* alloc ph page */
			region_alloc(e, (void *)ph->p_va, ph->p_memsz);
			/* ph->p_filesz bytes  starting at 'binary + ph->p_offset'  
			   copied to virtual address ph->p_va
			 */
...
env_create(uint8_t *binary, size_t size, enum EnvType type)
{
	...
	lcr3(PADDR(newenv_store->env_pgdir));
...
env_run(struct Env *e)
{
	...
	/* Step 1 */
	if (e != curenv) {
		/* if it is ENV_RUNNING, it can be not_runnable */
	 ...
	 * registers and drop into user mode 
	 */
