...
trap_init(void)
{
	...
	/* 10 invalid task switch segment */
...
trap_dispatch(struct Trapframe *tf)
{
	...
	if (trapno == T_PGFLT) {
		...
		return;
	} else if (trapno == T_BRKPT) {
		monitor(tf);
		return;
	} else if (trapno == T_DEBUG) {
		monitor(tf);
		return;
...
page_fault_handler(struct Trapframe *tf)
{
	...
	// LAB 3: Your code here.
	/* error code: U/S W/R P */
	/* 0x4: user mode bit */
	/* its wrong because tf_err 0x4 represents the page's pos, not who call
	if (!(tf->tf_err & 0x4)) {
...
void syscall_handler(struct Trapframe *tf) {
	tf->tf_regs.reg_eax = syscall(tf->tf_regs.reg_eax, tf->tf_regs.reg_ebx,
				tf->tf_regs.reg_ecx, tf->tf_regs.reg_edx, tf->tf_regs.reg_esi,
				tf->tf_regs.reg_edi);
}