...
static int 
sys_e1000_send(char *data, int len)
{
	return E1000_send(data, len);
}

static int
sys_e1000_receive(char *buf)
{
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	if (syscallno == SYS_cputs) {
		...
		res = sys_time_msec();
	} else if (syscallno == SYS_e1000_send) {
		res = sys_e1000_send((char *)a1, (int)a2);
	} else if (syscallno == SYS_e1000_receive) {
		res = sys_e1000_receive((char *)a1);
	} else {
		res = -E_INVAL;
	}
