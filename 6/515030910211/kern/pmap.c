...
mem_init(void)
{
	...
	check_page_alloc();
	//panic("Part1 finished!\n");
	check_page();
	//panic("Part2 finished!\n");
	...
	int perm = PTE_U;
	boot_map_region(kern_pgdir, va, size, pa, perm);
	...
	/* l dont know how to guard this section... */
	...
	// Your code goes here:
	va = KERNBASE;
	size = IOMEMBASE - KERNBASE;
	pa = 0;
	perm = PTE_W;
	/* Amazing bug!!!!!! */
	//boot_map_region_large(kern_pgdir, va, size, pa, perm);
	boot_map_region(kern_pgdir, va, size, pa, perm);

	/* enable page size extension before loading cr3 */
	cr4 = rcr4();
	cr4 |= CR4_PSE;
	lcr4(cr4);
...
mem_init_mp(void)
{
	...
	for (; i < NCPU; i++) {
		va = KSTACKTOP - KSTKSIZE - i * (KSTKSIZE + KSTKGAP);
		size = KSTKSIZE;
...
page_init(void)
{
	...
	size_t cur_page = (size_t)PGNUM(PADDR(boot_alloc(0)));
	...
	/* already in use for page tables and other data structures */
	for (; i < cur_page; i++) {
		pages[i].pp_ref = 1;
		pages[i].pp_link = NULL;
	}
...
page_alloc(int alloc_flags)
{
	...
	/* find a free page and remove it from free list */
	if (page_free_list) {
...
page_free(struct Page *pp)
{
	// Fill this function in
	if (pp->pp_ref != 0) {
		panic("page_free(): should only be called when pp->pp_ref reaches 0.\n");
	}
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	physaddr_t phy_pte = (physaddr_t)NULL;
	/* exist */
	if (pde & PTE_P) {
		phy_pte = PTE_ADDR(pde);
	} else {
		/* The relevant page table page might not exist yet */
		/* and create == false, then pgdir_walk returns NULL. */
		if (create == 0) {
			return result;
		} else {
			...
			if (!new_page) {
				...
				new_page->pp_ref++;
				phy_pte = page2pa(new_page);

				/* permission */
				pgdir[PDX(va)] = phy_pte | PTE_P | PTE_U | PTE_W;
			}
		}
	}

	result = (pte_t *)KADDR(phy_pte) + PTX(va);
	return result;
}
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	// Fill this function in
	if (size % PGSIZE != 0) {
		panic("boot_map_region(): size must be a multiple of PGSIZE\n");
	}

	size_t i;
	for (i = 0; i < size / PGSIZE; i++) {
		uintptr_t offset = i * PGSIZE;
		pte_t *pte = pgdir_walk(pgdir, (char *)va + offset, 1);
...
void
boot_map_region_pci(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	// Fill this function in
	if (size % PGSIZE != 0) {
		panic("boot_map_region(): size must be a multiple of PGSIZE\n");
...
boot_map_region_large(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	// Fill this function in
	if (size % PTSIZE != 0) {
		panic("boot_map_region_large(): size must be a multiple of PTSIZE\n");
	}

	size_t i;
	for (i = 0; i < size / PTSIZE; i++) {
		uintptr_t offset = i * PTSIZE;
		pgdir[PDX(va + offset)] = (pa + offset) | perm | PTE_P | PTE_PS; 
	}
}
...
page_insert(pde_t *pgdir, struct Page *pp, void *va, int perm)
{
	...
	if (!pte) {
		...
		/* incre pp_ref firstly for the corner-case */
		/* pp->pp_ref should be incremented if the insertion succeeds */ 
		pp->pp_ref++;
		/* If there is already a page mapped at 'va', it should be page_remove() */
		page_remove(pgdir, va);
		/* he permissions (the low 12 bits) of the page table entry should be set to 'perm|PTE_P' */
...
page_remove(pde_t *pgdir, void *va)
{
	...
	struct Page *rm_page = page_lookup(pgdir, va, &pte_store);
	/*  If there is no physical page at that address, silently does nothing */
	if (!rm_page) {
		return;
	} else {
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	/* align */
	uintptr_t va_aligned = (uintptr_t)ROUNDDOWN(va, PGSIZE);
	size_t len_aligned = (size_t)ROUNDUP(len, PGSIZE);

	uintptr_t off = 0;
	while (off < len_aligned) {
		/* (1) the address is below ULIM */
		if (va_aligned + off >= ULIM) {
			/* for print first addr in invalid page */
			if ((uintptr_t)va > va_aligned + off) {
				user_mem_check_addr = (uintptr_t)va + off;
			} else {
				user_mem_check_addr = va_aligned + off;
				//cprintf("va = %x\n", va);
				//cprintf("off = %u\n", off);
			}
			return -E_FAULT;
		...
		pte_t *pte = pgdir_walk(env->env_pgdir, (void *)(va_aligned + off), 0);
