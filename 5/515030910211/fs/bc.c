...
bc_pgfault(struct UTrapframe *utf)
{
	...
	r = sys_page_alloc(0, aligned_addr, PTE_P | PTE_U | PTE_W);
	if (r < 0) {
		panic("bc_pgfault(): %e\n", r);
	}

	r = ide_read(blockno * BLKSECTS, aligned_addr, BLKSECTS);
	if (r < 0) {
		panic("bc_pgfault(): %e\n", r);
	}
...
flush_block(void *addr)
{
	...
	int res;
	void *aligned_addr = ROUNDDOWN(addr, PGSIZE);
	if (va_is_mapped(aligned_addr) && va_is_dirty(aligned_addr)) {
		res = ide_write(blockno * BLKSECTS, aligned_addr, BLKSECTS);
		if (res < 0) {
			panic("flush_block(): %e\n", res);
		}

		res = sys_page_map(0, aligned_addr, 0, aligned_addr, PTE_P | PTE_U | PTE_W);
		if (res < 0) {
			panic("flush_block(): %e\n", res);
		}
	}
