...
serve_read(envid_t envid, union Fsipc *ipc)
{
	...
	struct OpenFile *of;
	int res;
	res = openfile_lookup(envid, req->req_fileid, &of);
	if (res < 0) {
		return res;
	} 

	uint32_t count = req->req_n;
	if (req->req_n > PGSIZE) {
		count = PGSIZE;
	}

	res = file_read(of->o_file, ret->ret_buf, count, of->o_fd->fd_offset);
	if (res < 0) {
		return res;
	}

	of->o_fd->fd_offset += res;
...
serve_write(envid_t envid, struct Fsreq_write *req)
{
	...
	res = file_write(of->o_file, req->req_buf, req->req_n, of->o_fd->fd_offset);
	if (res < 0) {
		return res;
	}
